...
// API route to insert data into database ของ นินิว
app.post('/insert', async (req, res) => {
    ...
    const query = `
        INSERT INTO StudentData (${columns})
        VALUES ${placeholders}
    `;
...
// เพิ่ม API สำหรับการลบโปรแกรมตาม studentId
// API สำหรับการลบข้อมูลนักเรียน
// 

// API สำหรับการลบข้อมูลนักเรียน
app.delete('/students/:studentid', async (req, res) => {
    const { studentid } = req.params; // รับค่า studentid จาก URL params
    ...
    try {
        const conn = await pool.getConnection();
        const result = await conn.query('DELETE FROM StudentData WHERE studentid = ?', [studentid]);
...
app.get('/api/get_assignment_detail', async (req, res) => {
    const { assignment_id } = req.query;  // ดึง assignment_id จาก URL parameter
    if (!assignment_id) {
    ...
    try {
      ...
      res.json(result);  // ส่งข้อมูลทั้งหมดที่เกี่ยวข้องกับ assignment_id
      conn.release();
    } catch (err) {
